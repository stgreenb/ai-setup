...
function normalizeTargetAgent(value: unknown): ('claude' | 'cursor' | 'codex' | 'github-copilot' | 'opencode')[] | undefined {
  if (Array.isArray(value)) return value;
  if (typeof value === 'string') {
    if (value === 'both') return ['claude', 'cursor'];
    if (['claude', 'cursor', 'codex', 'github-copilot', 'opencode'].includes(value)) return [value as 'claude' | 'cursor' | 'codex' | 'github-copilot' | 'opencode'];
  }
  return undefined;
}
